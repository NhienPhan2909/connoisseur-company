...
def get_review(restaurant_name: str) -> str:
    ...
    # Build a lookup from itemId to restaurant name, since reviews only
    # reference restaurants by itemId rather than storing the name directly
    id_to_name = {r["itemId"]: r["name"] for r in restaurants}

    # Find the matching review by resolving each review's itemId to a name
    matching_review = None
    matched_name = None
    for review in reviews:
        name = id_to_name.get(review["itemId"], "")
    ...
    return json.dumps(
        {
            ...
            "reviewer": matching_review["userId"],
            "rating": matching_review["rating"],
            "review_text": matching_review["text"],
            "image_description": matching_review.get("image_captions", "N/A"),
